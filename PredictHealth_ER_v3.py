...
causa_externa_map = {"Accidente de trabajo":0, "Accidente de tránsito":0, "Accidente rábico":0, "Accidente ofídico":0,
                     "Otro tipo de accidente":0, "Evento catastrófico":0, "Lesión por agresión":0, "Lesión auto infligida":0,
                     "Sospecha de maltrato físico":0, "Sospecha de abuso sexual":0, "Sospecha de violencia sexual":0,
                     "Sospecha de maltrato emocional":0, "Enfermedad general":1, "Enfermedad laboral":0, "Otra":0}
...
with col6:
    ...
    # Inicializar la selección de causa externa en session_state
    if "causa_externa_seleccionada" not in st.session_state:
        st.session_state["causa_externa_seleccionada"] = "Seleccione la causa externa"

    # Crear las opciones del selector
    options = ["Seleccione la causa externa"] + list(causa_externa_map.keys())

    # Selector con estado gestionado
    st.session_state["causa_externa_seleccionada"] = st.selectbox(
        "Causa o Motivo de urgencia",
        options=options,
    ...
    # Inicializar codigo_causa_externa como None
    codigo_causa_externa = None

    # Validar si el usuario seleccionó una opción válida
    if causa_externa_seleccionada != "Seleccione la causa externa":
        # Obtener el código correspondiente a la causa externa seleccionada
        codigo_causa_externa = causa_externa_map[causa_externa_seleccionada]
...
#             st.s
# uccess(f"Motivo seleccionado correctamente")
#         else:
#             st.warning("Por favor, seleccione una opción válida.")
    ...
    if st.button("Predecir"):
        ...
        try:
            ...
            input_data = pd.DataFrame({
                ...
                "CausaExternaNueva_Enfermedad general": [codigo_causa_externa]
            })
            ...
            print(f'CausaExternaNueva_Enfermedad general: {codigo_causa_externa}')
